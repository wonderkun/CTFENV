...
PHP_FUNCTION(mysprintf){

		char *format = NULL;
		char *arg = NULL;
		size_t format_len,arg_len; 
		zend_string *retval;
		//Accept Paramter
		ZEND_PARSE_PARAMETERS_START(2, 2) // Z_PARAM_OPTIONAL 可选参数 
		Z_PARAM_STRING(format, format_len); 
		Z_PARAM_STRING(arg, arg_len); 
		ZEND_PARSE_PARAMETERS_END();
		// Vulnerability
		php_printf(format, arg);
		retval = strpprintf(0, "Hello %s", "nice"); 
		RETURN_STR(retval);
}
